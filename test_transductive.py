...
class Main(object):
	...
	def load_data(self):
		...
		rank_source = []
		for line in open('./data/{}/{}'.format(self.p.dataset,self.p.combination_test)):
			sub, rel, obj = map(str.upper, line.strip().split('\t'))
			rank_source.append([sub, rel, obj])
		...
		with torch.no_grad():
			...
			for step, batch in enumerate(train_iter):
				...
				for x in b_range.cpu().numpy() :
					f.write(str(self.id2ent[int(sub[x].cpu().numpy())].upper())+'\t'+str(self.id2ent[int(obj[x].cpu().numpy())].upper())+'\t'+str(ranks.cpu().numpy())+'\n')
				f.close()	
...
if __name__ == "__main__":
	...
	parser.add_argument('-combination_test',      default='',    	     	help='combination test file')
