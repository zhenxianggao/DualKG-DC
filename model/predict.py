...
class GPKG_EMBEDD(BaseModel):
	def __init__(self, edge_index_base, edge_type_base, edge_index_upper, edge_type_upper, num_rel, params=None):
		...
		if self.p.num_bases > 0:
			...
			self.conv_upper_1 = GPKGConvUpper(self.p.gcn_dim,    self.p.embed_dim,    num_rel, act=self.act, params=self.p) 
			self.conv_upper_2 = GPKGConvUpper(self.p.gcn_dim,    self.p.embed_dim,    num_rel, act=self.act, params=self.p) 
		else:
	...
	def forward_embedd(self, label, sub, rel, drop_base1, drop_base2, drop_upper1, drop_upper2):
		...
		if label == 'upper' :
			...
			x, r	= self.conv_upper_1(x, self.edge_index_upper, self.edge_type_upper, rel_embed=r) 	
			x	= drop_upper1(x) 	
			x, r	= self.conv_upper_2(x, self.edge_index_upper, self.edge_type_upper, rel_embed=r) 	
			x	= drop_upper2(x) 							
